...
class nightmare {
  ...
  constructor(options) {
    ...
    this.namespaces.forEach(function(name) {
    ...
    Object.keys(this.childActions).forEach(function(key) {
  ...
  getInstance(){
      ...
      return new this.options.Promise(function(success, failure) {success(self);})
  }

  goto(browserName, url, headers) {
    ...
    this.queue(browserName, function(fn) {
      self.child.call('goto', browserName, url, headers, this.options.gotoTimeout, fn);
    })
    return this;
  }

  evaluate_now(windowName, js_fn, done) {
    ...
    //console.log(windowName,source, done)
    this.child.call('javascript', windowName, source, done);
    return this;
  };
  ...
  action() {
    ...
    if (parentfn) {
      if (typeof parentfn === 'function') {
        ...
        if (!~this.namespaces.indexOf(name)) {
          this.namespaces.push(name);
        }
        this[name] = function() {
          var self = this;
          return keys(parentfn).reduce(function(obj, key) {
            obj[key] = this.queued(name, parentfn[key]).bind(self)
            return obj;
          }, {});
        }
      }
    ...
    if (childfn) {
      if (typeof childfn === 'function') {
        this.childActions[name] = childfn;
      } else {
        for (var key in childfn) {
          this.childActions[name + '.' + key] = childfn;
        }
      }
    }
  }
  ...
  run(fn) {
    ...
    function next(err, res) {
      ...
      var args = item[1] || [];
      var method = item[0];
      let wName = (typeof(args[0]) == 'string')? args[0]: null;

      function after(err, res) {
        err = err || self.die;
        var args = sliced(arguments);
        if (self.child) {
          self.child.call('continue', wName, () => next.apply(self, args));
        } else {
...
Nightmare.prototype.goto = function(windowName, url, headers) {
  debug('queueing action "goto" for %s', url);
  var self = this;
  ...
  this.queue(function(fn) {
    ...
    self.child.call('goto', windowName, url, headers, this.options.gotoTimeout, fn);
  });
  return this;
};
...
Nightmare.prototype.run = function(fn) {
  ...
  function next(err, res) {
    ...
    var args = item[1] || [];
    var method = item[0];
    console.log(this.caller, String(once(after)));
    args.push(once(after));
...
Nightmare.prototype.evaluate_now = function(windowName, js_fn, done) {
  var args = Array.prototype.slice.call(arguments).slice(2).map(a => {
    return {
      argument: JSON.stringify(a)
    };
